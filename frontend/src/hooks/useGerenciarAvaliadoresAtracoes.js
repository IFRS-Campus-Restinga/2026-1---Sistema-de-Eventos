...
export default function useGerenciarAvaliadoresAtracoes(eventoId) {
    ...
    const extrairDadosParticipacao = (atracao) => {
        const fontesAutoria =
            Array.isArray(atracao?.autorias) && atracao.autorias.length
                ? atracao.autorias
                : Array.isArray(atracao?.equipe) && atracao.equipe.length
                  ? atracao.equipe
                  : [];
    ...
    const existemAtracoesElegiveisParaAtribuicao = useMemo(
        () => (atracoes || []).some(trabalhoElegivelParaAtribuicao),
        [atracoes, eventosMap],
    );

    const criarRecomendadosAutomaticos = (atracao, usuariosLista) => {
        const autorData = extrairDadosParticipacao(atracao);
        const atuaisSet = new Set(
            (atracao.avaliadores || []).map((av) =>
                String(av.perfil_id || av.id),
            ),
        );

        const limiteRaw =
            typeof atracao.modalidade === 'object'
                ? atracao.modalidade?.limite_avaliadores
                : modalidadesMap?.[atracao.modalidade]?.limite_avaliadores;
        ...
        const sugestoesArea = gerarSugestoesPorArea(candidatos, atracao);
